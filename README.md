...
- ✅ 用户认证与授权（JWT，基于角色的访问控制）
  - 邮箱密码登录
  - 手机号验证码登录（自动创建用户）
    - ⚠️ **注意**：当前为模拟实现，生产环境需接入短信服务商（详见 [短信接入指南](./docs/SMS_INTEGRATION.md)）
...
**测试状态**:
- ✅ **集成测试**: 全部通过（5/5），修复了邮箱重复和 HTTP 状态码问题
- ✅ **E2E 测试**: 27 个通过，3 个跳过（手机验证码测试，测试环境限制）
- ⚠️ **单元测试**: 部分跳过（需要接口重构），实际功能通过集成测试验证
...
- [**短信验证码接入指南**](./docs/SMS_INTEGRATION.md) - 如何接入短信服务商（阿里云、腾讯云等）
